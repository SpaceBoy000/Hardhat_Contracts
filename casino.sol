...
contract Casino is Ownable{
    ...
    function compraTokens(uint256 _numTokens) public {
        ...
        if  (token.balanceOf(address(this)) < _numTokens*1e18){
            token.mint(_numTokens*1e18);
        }
        ...
        token.transfer(msg.sender, _numTokens*1e18);
    }
    ...
    function devolverTokens(uint _numTokens) public {
        ...
        require(_numTokens*1e18 <= token.balanceOf(msg.sender), "No tienes los tokens que deseas devolver");
        // El usuario transfiere los tokens al Smart Contract
        token.transferFrom(msg.sender, address(this), _numTokens * 1e18);
    ...
    function jugarRuleta(uint _start, uint _end, uint _tokensBet) public returns (uint, bool, uint){
        require(_tokensBet*1e18 <= token.balanceOf(msg.sender));
        require(_tokensBet > 0);
        uint random = uint(uint(keccak256(abi.encodePacked(block.timestamp))) % 14);
        uint tokensEarned = 0;
        bool win = false;
        token.transferFrom(msg.sender, address(this), _tokensBet*1e18);
        if ((random <= _end) && (random >= _start)) {
            ...
            if  (token.balanceOf(address(this)) < tokensEarned*1e18){
                token.mint(tokensEarned*1e18);
            }
            token.transfer(msg.sender, tokensEarned*1e18);
        }
        addHistorial("Roulete", _tokensBet, tokensEarned, msg.sender);
        emit RouletteGame(random, win, tokensEarned);

        return (random, win, tokensEarned);
    }
